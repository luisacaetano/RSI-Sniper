#!/bin/bash
# RSI Sniper - Painel de Controle (Linux)
...
if [ -z "$PANEL_FOUND" ]; then
    PROG_PANEL="$HOME/.wine/drive_c/Program Files/MetaTrader 5/MQL5/Include/MWM/rsi_panel.py"
    if [ -f "$PROG_PANEL" ]; then
        PANEL_DIR="$HOME/.wine/drive_c/Program Files/MetaTrader 5/MQL5/Include/MWM"
        PANEL_FOUND="1"
    fi
fi
